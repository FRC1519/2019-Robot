...
public class RocketFrontToLoadingStationFast extends CommandGroup {
  ...
  public RocketFrontToLoadingStationFast(Autonomous.StartOn startSide) {
    ...
    addSequential(new DriveStraightOnHeading(-0.5, 6, Autonomous.chooseAngle(startSide, 38.0)));
    ...
    addSequential(new DriveStraightOnHeading(0.9, 36, Autonomous.chooseAngle(startSide, 180.0)));

    // drive a bit farther straight towards loading station before turning on
    // auto-alignment

    addSequential(new DriveStraightOnHeading(0.8, 24, Autonomous.chooseAngle(startSide, 180.0)));

    // TODO: consider modifying AutoAlign to use range to determine speed of
    // drive, or at least the deceleration
    ...
    // addSequential(new AutoAlignWithSlowdownUntilAtWall(3.0,
    // Targeting.TargetPosition.CENTER_MOST));
